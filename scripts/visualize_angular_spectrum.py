...
def save_figure(key, mat, dest, frame_shift):
    ...
    plt.yticks(yp, ["%d" %(num_doas - d) for d in yp])
    plt.xlabel('Time(s)')
    plt.ylabel('DoA')
    plt.colorbar()
    plt.savefig(dest)
...
def run(args):
    if not os.path.exists(os.path.basename(args.save_to)):
        os.makedirs(os.path.basename(args.save_to))
...
if __name__ == '__main__':
    ...
    parser.add_argument('feature_mat', type=str,
    ...
    parser.add_argument('--save-to', type=str, default="figure.png", dest="save_to", 
