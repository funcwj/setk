...
if ! $noise; then
    ...
    $cmd JOB=1:$nj $logg_dir/compute_${name}_${mask}.JOB.log \
        compute-masks --verbose=2 --config=$stft_config scp:$logg_dir/${name}_noise.JOB.scp \
    ...
    $cmd JOB=1:$nj $logg_dir/compute_${name}_${mask}.JOB.log \
        compute-masks --verbose=2 --config=$stft_config scp:$logg_dir/${name}_clean.JOB.scp \
