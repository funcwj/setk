...
find `cd $noisy_dir; pwd` -name "*.wav" | awk -F '/' '{split($NF, a, "."); printf "%s%s\t%s\n", a[1], a[2], $0}' | sort -k1 > $mask_dir/wav.scp
find `cd $clean_dir; pwd` -name "*.wav" | awk -F '/' '{split($NF, a, "."); printf "%s%s\t%s\n", a[1], a[2], $0}' | sort -k1 > $mask_dir/clean.scp
find `cd $noise_dir; pwd` -name "*.wav" | awk -F '/' '{split($NF, a, "."); printf "%s%s\t%s\n", a[1], a[2], $0}' | sort -k1 > $mask_dir/noise.scp
