...
#else
#error "You must add definitions in CMakeLists.txt(-DHAVE_OPENBLAS or -DHAVE_CLAPACK)"
#endif

inline void cblas_CZscal(const int N, const void *alpha, float *data, const int inc) {
    cblas_cscal(N, alpha, data, inc);
}


inline void cblas_CZscal(const int N, const void *alpha, double *data, const int inc) {
...
inline void cblas_CZdot(const int N, const float *X,
...
inline void cblas_CZdot(const int N, const double *X,
...
inline void cblas_CZgemm(const void *alpha,
  ...
  cblas_cgemm(CblasRowMajor, static_cast<CBLAS_TRANSPOSE>(transA), 
              ...
              alpha, Adata, a_stride >> 1, Bdata, b_stride >> 1,
              beta, Mdata, stride >> 1); 
}
...
inline void cblas_CZger(MatrixIndexT num_rows, MatrixIndexT num_cols, void *alpha,
                       const float *xdata, MatrixIndexT incX, const float *ydata,
                       MatrixIndexT incY, float *Mdata, MatrixIndexT stride, bool conj) {
    if (conj)
        cblas_cgerc(CblasRowMajor, num_rows, num_cols, alpha, xdata, 1, ydata, 1, Mdata, stride >> 1);
    else
        cblas_cgeru(CblasRowMajor, num_rows, num_cols, alpha, xdata, 1, ydata, 1, Mdata, stride >> 1);

}

inline void cblas_CZger(MatrixIndexT num_rows, MatrixIndexT num_cols, void *alpha,
                       const double *xdata, MatrixIndexT incX, const double *ydata,
                       MatrixIndexT incY, double *Mdata, MatrixIndexT stride, bool conj) {
    if (conj)
        cblas_zgerc(CblasRowMajor, num_rows, num_cols, alpha, xdata, 1, ydata, 1, Mdata, stride >> 1);
    else
        cblas_zgeru(CblasRowMajor, num_rows, num_cols, alpha, xdata, 1, ydata, 1, Mdata, stride >> 1);
...
inline void cblas_CZgemv(MatrixTransposeType trans, MatrixIndexT num_rows,
                        ...
                        MatrixIndexT incX, void *beta, float *ydata, MatrixIndexT incY) {
    cblas_cgemv(CblasRowMajor, static_cast<CBLAS_TRANSPOSE>(trans), num_rows,
                num_cols, alpha, Mdata, stride >> 1, xdata, incX, beta, ydata, incY);

}

inline void cblas_CZgemv(MatrixTransposeType trans, MatrixIndexT num_rows,
                        MatrixIndexT num_cols, void *alpha, const double *Mdata,
                        MatrixIndexT stride, const double *xdata,
                        MatrixIndexT incX, void *beta, double *ydata, MatrixIndexT incY) {
...
inline void clapack_CZgetrf(KaldiBlasInt *num_rows, KaldiBlasInt *num_cols, 
                            ...
                            KaldiBlasInt *result) {
    cgetrf_(num_rows, num_cols, reinterpret_cast<KaldiComplexFloat*>(Mdata), stride, pivot, result);
}

inline void clapack_CZgetrf(KaldiBlasInt *num_rows, KaldiBlasInt *num_cols, 
                            double *Mdata, KaldiBlasInt *stride, KaldiBlasInt *pivot, 
                            KaldiBlasInt *result) {
    zgetrf_(num_rows, num_cols, reinterpret_cast<KaldiComplexDouble*>(Mdata), stride, pivot, result);
}

// int cgetri_(integer *n, complex *a, integer *lda, integer *
//             ipiv, complex *work, integer *lwork, integer *info);
inline void clapack_CZgetri(KaldiBlasInt *num_rows, float *Mdata, KaldiBlasInt *stride,
                           KaldiBlasInt *pivot, float *work, 
                           KaldiBlasInt *lwork, KaldiBlasInt *result) {
...
inline void clapack_CZgetri(KaldiBlasInt *num_rows, double *Mdata, KaldiBlasInt *stride,
                           KaldiBlasInt *pivot, double *work, 
                           KaldiBlasInt *lwork, KaldiBlasInt *result) {
    zgetri_(num_rows, reinterpret_cast<KaldiComplexDouble*>(Mdata), stride, pivot, 
                      reinterpret_cast<KaldiComplexDouble*>(work), lwork, result);
}
