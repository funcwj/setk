...
int main(int argc, char *argv[])
{
    try {
        ...
        const char *usage =
            ...
            "table of vectors\n"
            "\n"
            "Usage: matrix-scale-rows [options] <vector-rspecifier> <matrix-rspecifier> <matrix-wspecifier>\n"
        ...
        for (; !vec_reader.Done(); vec_reader.Next()) {
            ...
            if (vector_dim != num_rows) {
              ...
              KALDI_VLOG(1) << "vector dim = " << vector_dim << ", matrix num_rows = "
                            << num_rows << ", scale first " << num_scale_rows << " rows";
            }

            mat.RowRange(0, num_scale_rows).MulRowsVec(scale.Range(0, num_scale_rows));
