...
void ComputeMasks(ShortTimeFTComputer &stft_computer, 
    ...
    stft_computer.Compute(noisy_data, NULL, &denumerator, NULL);

    if (type == "irm" || type == "wiener") {
        // denumerator is noisy, so do not need to add them
...
int main(int argc, char *argv[]) {
    try{
        const char *usage = 
            ...
            "Usage:  compute-masks [options...] <noisy-rspecifier> <clean-rspecifier> <mask-wspecifier>\n"
            "   or:  compute-masks [options...] <noisy-rxfilename> <clean-rxfilename> <mask-wxfilename>\n"
        ...
        bool wx_binary = false, enable_scale = false;
        ...
        // po.Register("enable-scale", &enable_scale, "Let inf-norm of sample vector be one");
        ...
        std::string noisy_in = po.GetArg(1), clean_in = po.GetArg(2), mask_out = po.GetArg(3);
        ...
        if (noisy_is_rspecifier != mask_is_wspecifier)
            KALDI_ERR << "Cannot mix archives with regular files";

        if (noisy_is_rspecifier != clean_is_rspecifier)
            KALDI_ERR << "Configure with noisy/clean must keep same";
        ...
        if (noisy_is_rspecifier) {
            ...
            for (; !noisy_reader.Done(); noisy_reader.Next()) {
                ...
                Matrix<BaseFloat> mask;
                ComputeMasks(stft_computer, noisy_data.Data(), clean_data.Data(), mask_type, &mask);
            ...
            bool binary;
            Input kn(noisy_in, &binary);
            Input kt(clean_in, &binary);

            WaveData noisy_data, clean_data;
            noisy_data.Read(kn.Stream());
